...
int VMemoryManager::storePage(TranslationEntry* entry) {
  ...
  if (entry->dirty) {
    entry->dirty = FALSE;
    swapFile->WriteAt(machine->mainMemory + (entry->physicalPage * PageSize), 
		      PageSize, entry->swapSector * PageSize);
    DEBUG('2', "S %d: %d\n", currentThread->space && currentThread->space->pcb ? currentThread->space->pcb->GetPID() : -1, 
	  entry->virtualPage);
  } else {
    DEBUG('2', "E %d: %d\n", currentThread->space && currentThread->space->pcb ? currentThread->space->pcb->GetPID() : -1, 
	  entry->virtualPage);
...
int VMemoryManager::loadPage(TranslationEntry* entry) {
  ...
  ASSERT(!entry->inMem); //it is not in memory now
  ...
  TranslationEntry *oldest;
    ...
    // 		Pick a page to remove from memQueue, using second chance FIFO or LRU

    // destPage stores the physical page to be used
//    DEBUG('1', "Swapping page %d->%d (%x) with %d->%d\n", oldest->virtualPage, oldest->physicalPage, oldest, entry->virtualPage, destPage);

    // code removed

    // Remove from TLB
    for (int i = 0; i < TLBSize; i++) {
      if (machine->tlb[i].virtualPage == oldest->virtualPage && machine->tlb[i].valid) {
	...
	for (int j = 0; j < currentThread->space->numPages; j++) {
	  if (pageTable[j].valid && pageTable[j].virtualPage == machine->tlb[i].virtualPage) {
  ...
  // Update the input paramter entry and add it to the queue.
   //code is removed
  ...
  if (entry->firstUse) {
    DEBUG('2', "Z %d: %d -> %d\n", currentThread->space && currentThread->space->pcb ? currentThread->space->pcb->GetPID() : -1, entry->virtualPage, entry->physicalPage);
